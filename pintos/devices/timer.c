...
static bool wake_less(const struct list_elem *a, const struct list_elem *b,
                      void *aux
                          UNUSED);  // 'sleep_list' 깨울 시각 오룸차순 위해
...
void timer_init(void) {
  ...
  list_init(&sleep_list);  // 1️⃣ sleep_list 초기화   (list_init:
                           // 라이브러리 함수)
}
...
void timer_sleep(int64_t ticks) {
  ...
  int64_t wake =
      timer_ticks() +
      ticks;  // 지금 시각(틱 수) + 얼마나 잘지 = "깨울 절대 시각"을 계산
  struct thread *cur = thread_current();  // 현재 스레드

  enum intr_level old = intr_disable();  // 현재 인터럽트 상태를 old에 저장 +
                                         // 인터럽트 off(방해 금지 모드)
  cur->wakeup_tick = wake;  // 내가 깰 시각

  list_insert_ordered(
      &sleep_list, &cur->elem, wake_less,
      NULL);  // sleep_list(잠자는 명단)에 A를 정렬 유지하며 삽입
...
static void timer_interrupt(struct intr_frame *args UNUSED) {
  ticks++;
  thread_tick();
  wake_sleepers();  // 1️⃣이번 tick에 깨어날 스레드들만
                    // O(k)으로 깨움
}
