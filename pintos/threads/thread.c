...
void thread_refresh_priority(struct thread *t) {
  ...
  for (struct list_elem *e = list_begin(&t->donations);  // 모든 기부자를 훑어
    ...
    if (donor->priority > newp)
      newp = donor->priority;  // 도너의 "현재 유효" 우선순위 사용
  }
  int oldp = t->priority;
  t->priority = newp;  // 새 유효 우선순위 커밋
  if (newp != oldp)
    ready_reinsert_if_needed(t);  // 값이 변했으면 -> READY면 큐 정렬 복구
}
...
tid_t thread_create(const char *name, int priority, thread_func *function,
  ...
  if (t->priority > thread_current()->priority)
    thread_yield();  // 2️⃣ 더 높은 애가 생기면 바로
                     // 양보(선점)
...
void thread_set_priority(int new_priority) {
  ...
  // cur->base_priority = new_priority;         // 3️⃣donation-one 대비:
  // 원래 우선순위도 갱신 cur->priority = new_priority;

  //  // 3️⃣ ready_list 맨 앞(최고 우선순위)이 나보다 높으면
  //  즉시 양보
  // if (!list_empty(&ready_list)) {
  //     struct thread *top = list_entry(list_front(&ready_list), struct thread,
  //     elem); if (top->priority > cur->priority)
  //        thread_yield();
  // }
  ...
  //   elem); if (t->priority > thread_current()->priority)
  // 	  thread_yield();                                    // ready에 나보다
  // 높은 애가 있으면 즉시 양보
}
...
static void schedule(void) {
  ...
  if (curr != next) {
    ...
    if (curr && curr->status == THREAD_DYING && curr != initial_thread) {
      ASSERT(curr != next);
      if (curr->is_waited) list_push_back(&destruction_req, &curr->elem);
    }
